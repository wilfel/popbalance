...
class Population:
    ...
    def redistribute_bins(self, d_old, d_new):
        N_old = self.N_dist.copy()
        M_old = self.N_dist * self.m
        M_solute_old = self.N_dist * self.m_solute
        M_water_old = self.N_dist * self.m_water

        N_new = np.zeros_like(N_old)
        M_new = np.zeros_like(M_old)
        M_solute_new = np.zeros_like(M_solute_old)
        M_water_new = np.zeros_like(M_water_old)

        d_new_nm = d_new * 1e9

        for i in range(self.grid.n_bins):
            # Find which bin the new diameter falls into (using edges, not centers)
            h = np.searchsorted(self.grid.edges, d_new_nm[i], side='right') - 1
            h = np.clip(h, 0, self.grid.n_bins - 1)

            # Move ALL particles from bin i to bin h
            N_new[h] += N_old[i]
            M_new[h] += M_old[i]
            M_solute_new[h] += M_solute_old[i]
            M_water_new[h] += M_water_old[i]

        self.N_dist = N_new

        mask = N_new > 0
        self.m = np.zeros_like(M_new)
        self.m_solute = np.zeros_like(M_solute_new)
        self.m_water = np.zeros_like(M_water_new)

        self.m[mask] = M_new[mask] / N_new[mask]
        self.m_solute[mask] = M_solute_new[mask] / N_new[mask]
        self.m_water[mask] = M_water_new[mask] / N_new[mask]
        self.d_dist = self.grid.centers.copy() * 1e-9  # reset to bin centers (m)

    def redistribute_bins_old(self, d_old, d_new):
        
        #Redistribute particles to grid after diameter change. Should only be run AFTER moved to dry/wet.
        
        widths = self.grid.widths
        N_old = self.N_dist.copy()
        N_new = np.zeros_like(N_old)    # store empty

        d_old = d_old * 1e9
        d_new = d_new * 1e9
        
        for i in range(self.grid.n_bins):   # for each bin
            
            Ni = N_old[i]   # original number of particles in that bin
            
            dd = (d_new[i] - d_old[i])    # change in particle diameter [m]
            frac = min(1.0, abs(dd) / widths[i])    # fraction that leaves based on the bin size; limit to 1

            d_exit = (d_old[i] + dd)

            h = np.searchsorted(self.grid.centers, d_exit) - 1  # find which index new d fits in
            h = np.clip(h, 0, self.grid.n_bins-1)   # limit index to length of array
        ...
        self.N_dist = N_new     # define new redistributed distribution
        
        self.N_dist = N_new

    def redistribute_bins_new(self, d_old, d_new):
