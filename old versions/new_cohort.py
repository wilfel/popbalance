...
Refactored to use Lagrangian cohort tracking for dry/nucleated particles.
Wet particles remain on a fixed size grid. Dry particles are tracked as
discrete cohorts — each cohort nucleated or dried at a specific time and
all particles within it share the same diameter, growing together via condensation.
"""
...
N_bins = 150        # number of bins
dt = 0.0000001      # time step (s)
total_time = 0.005  # total simulation time (s)
plot_interval_seconds = 0.00001
plot_interval = int(plot_interval_seconds / dt)
init_total_num = 5.63e7
init_mean = 825     # initial geometric mean diameter (nm)
init_sd = 1.68      # initial geometric standard deviation
...
class SizeGrid:
    """
    Static properties of the logarithmic size grid used for wet particles and plotting.
    """
...
# =============================================================================
# WET POPULATION (bin-based, unchanged)
# =============================================================================

class WetPopulation:
    """
    Bin-based representation of the wet droplet population.
    """
    ...
    def update_diameter(self, dm_water_dt, dm_SA_dt, dt):
        self.m_water  = np.maximum(self.m_water  + dm_water_dt * dt, 0)
        ...
        rho_mix[mask] = self.m[mask] / (
            self.m_solute[mask] / rho_solute + self.m_water[mask] / rho_water)
...
# =============================================================================
# COHORT POPULATION (Lagrangian, replaces dry Population)
# =============================================================================
...
class CohortPopulation:
    ...
    def merge_similar_cohorts(self, tol=0.01):
        """
        Merge cohorts within `tol` fractional diameter of each other to keep
        the cohort list length manageable. Conserves total number and mass.
        """
        if len(self.cohorts) < 2:
            return
        self.cohorts = [c for c in self.cohorts if c.N > 0]
        self.cohorts.sort(key=lambda c: c.d)
        merged = [self.cohorts[0]]
        for c in self.cohorts[1:]:
            prev = merged[-1]
            if abs(c.d - prev.d) / max(prev.d, 1e-30) < tol:
                total_N = prev.N + c.N
                # Number-weighted average of per-particle quantities
    ...
    @property
    def total_mass(self):
        return sum(c.N * c.m for c in self.cohorts)
...
class DryingModel:
    ...
    # -------------------------------------------------------------------------
    # Water evaporation (wet population)
    # -------------------------------------------------------------------------
    def evaporate_water(self, wet_pop):
        ...
        P_sat  = 0.0313
        P_tot  = 2.04138
        y_sat  = RH_sat * P_sat * 18.016 / (
                     RH_sat * P_sat * 18.016 + (P_tot - RH_sat  * P_sat) * 28.97)
        y_bulk = RH_bulk* P_sat * 18.016 / (
                     RH_bulk* P_sat * 18.016 + (P_tot - RH_bulk * P_sat) * 28.97)
        v_g    = 0.63662
        Re     = rho_g * v_g * d / mu_g
        Sc     = mu_g / (rho_g * Dv)
        ...
        mask   = d > 0
        dm_dt[mask] = -(A_d[mask] * rho_g * Dv * (y_sat - y_bulk) * Sh[mask]) / d[mask]
        return dm_dt

    # -------------------------------------------------------------------------
    # SA mass flux for wet particles
    # -------------------------------------------------------------------------
    def wet_SA_massloss(self, wet_pop, gas):
        not_empty = wet_pop.N_dist > 0
        d_m  = np.where(not_empty, wet_pop.d_dist, 0.0)
        T    = 298.15;  R = 8.314;  M_sa = 0.11809;  M_w = 0.01806
        A_d  = 4 * np.pi * (d_m / 2)**2
        p_sat_SA    = 1.95e-3
        conc_SA_liq = wet_pop.x_solute * wet_pop.frac_SA * 1000
        molefrac    = (conc_SA_liq / M_sa) / (
                          (conc_SA_liq / M_sa) + ((1000 - conc_SA_liq) / M_w))
        p_SA_surface= molefrac * p_sat_SA
        p_SA_bulk   = gas.conc_SA_gas * R * T / M_sa
        rho_g  = 2.416;  mu_g = 18.37e-6;  D_SA = 9.52e-6;  v_g = 0.63662
        Re_SA  = rho_g * v_g * d_m / mu_g
        Sc_SA  = mu_g / (rho_g * D_SA)
        Sh_SA  = 2 + 0.552 * Re_SA**(1/2) * Sc_SA**(1/3)
        k_mass = np.where(not_empty, Sh_SA * D_SA / np.where(d_m > 0, d_m, 1), 0.0)
        C_SA_surface = p_SA_surface * M_sa / (R * T)
        C_SA_bulk    = p_SA_bulk    * M_sa / (R * T)
        dm_SA_dt = -k_mass * A_d * (C_SA_surface - C_SA_bulk)
        return dm_SA_dt

    # -------------------------------------------------------------------------
    # Condensation onto cohorts (dry Lagrangian particles)
    # -------------------------------------------------------------------------
    def grow_cohorts(self, cohort_pop, gas, dt):
        """
        Apply condensation to every cohort using Fuchs-Sutugin correction.
        Updates each cohort's d and m in-place.
        Returns total mass (kg) transferred from gas to cohorts this timestep.
        """
        if not cohort_pop.cohorts:
            return 0.0

        T      = 298.15;  R = 8.314;  M_sa = 0.11809
        D_SA   = 2e-6
        alpha  = 1.0
        rho_sa = 1560.0
    ...
    # -------------------------------------------------------------------------
    # Wet → dry transfer (dried wet particles become new cohorts)
    # -------------------------------------------------------------------------
    ...
    def advance(self, wet_pop, cohort_pop, gas, dt, t):
        ...
        wet_pop.update_diameter(dm_water_dt, dm_SA_dt, dt)

        # --- Dry cohorts: grow via condensation ---
        mass_cond = self.grow_cohorts(cohort_pop, gas, dt)
...
for step in range(grid.n_steps):
    ...
    if step % plot_interval == 0:
        ...
        print(f"t={t*1000:.3f} ms | cohorts={n_cohorts:4d} | "
              f"gas SA={gas.conc_SA_gas:.3e} kg/m³ | "
              f"dry N={cohort_pop.total_N:.3e}")
